...
| **マイク権限拒否** | 影響なし | シャドーイングはお手本再生のみ（録音なし）。完了条件を満たせないためスキップ導線を出す。スキップは `completedCount` に入れない。瞬間英作文はタイプ入力で完了可 | 権限を要求しない設計のため影響なし | |
| **音声認識権限拒否** | 影響なし | シャドーイングは録音・自己再生（完了カウント可）。瞬間英作文はタイプ入力。ASR 不可 / 認識エラーは未採点完了（Attempt は追記、`.fail` の ReviewEvent は書かない） | 同上 | |
...
| 名前空間 | `onboarding.*` / `home.today.*` / `home.goal.*` / `home.recovery.*` / `streak.*` / `review.session.*` / `review.summary.*` / `notification.*` / `settings.*` / `home.drive.*` / `drive.*`（既存に追加） |
...
flowchart TD
  ...
  subgraph drive ["運転中 完全音声進行"]
    ...
    SSeq --> Next
    Next -->|残あり| Item
    Next -->|時間切れ or プラン消化| Outro["終了アナウンス 完了数"]
    Int["割り込み: ナビ / 電話 / BT切断"] -.->|自動一時停止| Item
  end
  subgraph post ["停車後"]
    Outro --> Note["ドライブノート 表現一覧 / 聞き直し / 通常レッスンへ"]
...
| 停車後 | アプリを開くとドライブノート（§10.5.3）。閉じるとホーム（ゴールリング / ストリークは反映済み） |
...
| セッション終了 | 自然終了時 1 回 | 完了数＋ドライブノートへの案内。手動停止では流さない |
...
| 長さ選択 | 4 択セグメント。タップ即反映・永続化（次回のすぐ始めるに使われる） |
...
| 終了 | 左上 44pt。確認ダイアログなし（完了分は保存済み。§2.4 と同じ理由）。ドライブノートへ |
...
| 項目 | 仕様 |
|------|------|
| 完了の記録 | Item 完了（§10.1 の定義）ごとに **未採点 `LessonAttempt` を追記**。§2.1 の「アイテム完了」定義そのままなので、ゴール進捗・ストリーク・`streak_day_recorded` / `goal_met` の一回性は既存の `appendAttemptEvaluatingHabit` がそのまま働く（二重実装しない） |
...
| 対象 | 指針 |
|------|------|
| 開始画面・ドライブノート | 通常画面の基準どおり（44pt、VoiceOver 順、Dynamic Type）。巨大開始ボタンは `accessibilityLabel` =「ドライブセッションを開始」 |
