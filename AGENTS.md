...
- `Packages/SnapSpeakCore` … **Foundation のみに依存する中核ドメイン**（`LanguageKit` / `ScoringKit` / `CompositionKit` / `SRSKit` / `ContentCore` / `AnalyticsCore` と実行可能 `contentlint`）。外部依存は swift-crypto のみ。**この Linux VM で `swift build` / `swift test` が完結する唯一の部分**。
- `Packages/SnapSpeakiOS` … Apple 専用（SwiftUI / SwiftData / AVFoundation / Speech / UIKit）。**Linux ではコンパイル不可**。検証は macOS CI（GitHub Actions の `ios-macos` ジョブ）のみ。
...
- iOS のユニットテスト（`PersistenceTests` / `ContentKitTests`）は **ホストアプリを使わないロジックテスト**にしてある。SwiftPM の静的ライブラリ製品を bundle_loader 経由で解決するとリンクに失敗するため、テストターゲットは必要な製品を直接リンクする（`App/project.yml` 参照）。
