...
- Cloud VM で beads（`bd`）を使う場合（標準環境にはプリインストールされていない）: `npm i -g --prefix ~/.local "@beads/bd"` の後 `export PATH="$HOME/.local/bin:$PATH"`。初回のみ `bd init -p ss --skip-agents --skip-hooks --non-interactive` を実行し、直後に `git log -1 --format='%s'` を確認して **`bd init` の自動コミットが作られていた場合のみ** `git reset --mixed HEAD~1 && git checkout -- .gitignore` で取り消す。`bd config set no-db true` を設定してよいが、bd 1.2.2 はローカルキャッシュ（`.beads/embeddeddolt` 等）を生成する。これらは gitignore 済みでコミットせず、課題の共有は従来どおり git 追跡の `.beads/issues.jsonl` を正本として行う（`bd dolt push` でリポジトリの remote に issue 履歴を書き込まない）。
