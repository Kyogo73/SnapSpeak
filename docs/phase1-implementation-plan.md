...
「1 マニフェスト内の複数 release」方式（arch §7.3 の正）で実装する。実 CDN（**Cloudflare R2** 採用。arch §8.1 参照）の払い出しとアップロード手順は Phase 1 後半（§6）。
...
| 実 CDN（Cloudflare R2）の払い出し・アップロード運用 | 外部依存。ロジック（ReleaseSelector / DownloadManager）はモックとローカルファイルで先行検証。バケット構成・キャッシュヘッダは arch §8.1 |
