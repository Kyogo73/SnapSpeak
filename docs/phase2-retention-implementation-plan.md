...
> **状態: 実装完了（記録）。** 本計画は実装 PR（レビュー 3 ラウンド）を経て develop にマージ済みであり、本書は**実装完了後の記録**として現状の実装に同期してある。レビュー反映で計画から差分が出た箇所（API シグネチャ・状態・追加キー）は各節に「実装差分」として追記した。§7 のコミット順は実績。既知の残課題と後続の品質改善は [quality-pass-plan.md](./quality-pass-plan.md) を参照。
